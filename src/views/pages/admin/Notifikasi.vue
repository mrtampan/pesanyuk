...
<template>
  <div id="data-list-thumb-view" class="data-list-container">
    ...
    <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage" search :data="notifications">
      ...
      <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
        ...
        <div class="flex flex-wrap-reverse items-center">
          ...
          <!-- ACTION - DROPDOWN -->
          <vs-button @click="deleteMultiple()" v-if="selected.length > 0 || selected.length < 0" class="p-3 mb-4 mr-4 rounded-lg cursor-pointer shadow-md flex items-center justify-between text-lg font-medium text-base text-white bg-danger">Hapus Notifikasi</vs-button>
          ...
          <div class="p-3 mb-4 mr-4 rounded-lg cursor-pointer shadow-md flex items-center justify-between text-lg font-medium text-base text-white bg-primary" @click.stop="addNewData()">
              ...
              <span class="ml-2 text-base text-white">Tambah Notifikasi</span>
          </div>
        </div>
        ...
        <vs-dropdown vs-trigger-click class="cursor-pointer mb-4 mr-4">
          <div class="p-4 border border-solid d-theme-border-grey-light rounded-full d-theme-dark-bg cursor-pointer flex items-center justify-between font-medium">
            <span class="mr-2">{{ currentPage * itemsPerPage - (itemsPerPage - 1) }} - {{ notifications.length - currentPage * itemsPerPage > 0 ? currentPage * itemsPerPage : notifications.length }} of {{ queriedItems }}</span>
       ...
       <template slot="thead">
        ...
        -->
        <vs-th sort-key="fitur">Nama Notifikasi</vs-th>
    ...
    <vs-popup class="crud uppercase" :title="titlePopup" :active.sync="popupActivo">
      <template>
        <h4 class="mb-5 font-bold">Informasi Notifikasi</h4>
        ...
        <div class="vx-row mb-6">
          ...
          <div class="vx-col sm:w-2/3 w-full">
            <vs-input class="w-full" icon-pack="feather" icon="icon-hp" name="url" icon-no-border v-model="url" v-validate="{ required: true, regex: /\d+(\.\d+)?$/ }"/>
...
import moduleDataNotifikasi from '@/store/admin/notifikasi/moduleNotifikasi.js' //penamaan moduleDataSupplier diganti sesual menui
...
export default {
  data () {
    return {
      ...
      popupData: {},
      titlePopup: 'Tambah',
      title: 'Log Notifikasi' // Ganti Sesuai jenis data CRUD
    }
  },
  components: {
    flatPickr
  ...
  computed: {
    ...
    notifications () { //ganti suppliers dengan menu yang baru
      return this.$store.state.dataNotifikasi.notifications //ganti suppliers dengan menu yang baru | dataSupplier wajib sama dengan bagian created > register module | ganti nama supplierswajib sama dengan state.js nya
    },
    queriedItems () {
      return this.$refs.table ? this.$refs.table.queriedResults.length : this.notifications.length //ganti suppliers dengan menu yang baru
    }
  },
  methods: {
    deleteMultiple () {
    ...
    acceptMultipleDelete () {
      for (let index = 0; index < this.selected.length; index++) {
        this.$store.dispatch('dataNotifikasi/removeItem', this.selected[index].id).catch(err => { console.error(err) })
      }
      this.$vs.notify({
        color: 'success',
      ...
      this.$validator.validateAll().then(result => {
        if (result) {
          ...
          if (this.id !== null && this.id >= 0) {
            this.$store.dispatch('dataNotifikasi/updateItem', obj).catch(err => { console.error(err) }) //dataSupplier harus sama dengan registrasi module
            this.$vs.notify({
              color: 'success',
              title: `Edit ${this.title} Berhasil`,
              text: `Data ${this.title} ${obj.notifications} berhasil ditambahkan` // obj.supplier ini bisa di ganti dengan variable di array obj di atas, wajarnya yang menginisialkan nama. contoh obj.nama
            })
          } else {
            delete obj.id
            this.$store.dispatch('dataNotifikasi/addItem', obj).catch(err => { console.error(err) }) //dataSupplier harus sama dengan registrasi module
            this.$vs.notify({
              color: 'success',
    ...
    acceptAlert () {
      this.$store.dispatch('dataNotifikasi/removeItem', this.popupData.id).catch(err => { console.error(err) })
      this.$vs.notify({
        color: 'success',
  ...
  created () {
    if (!moduleDataNotifikasi.isRegistered) {
      this.$store.registerModule('dataNotifikasi', moduleDataNotifikasi) //dataSupplier ganti dengan dataNamamenu
      moduleDataNotifikasi.isRegistered = true
    }
    this.$store.dispatch('dataNotifikasi/fetchDataNotificationItems')
  },
  mounted () {
    this.isMounted = true
  }
