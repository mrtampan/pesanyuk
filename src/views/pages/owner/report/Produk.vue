...
<!-- ADD NEW -->
       ...
       <template slot="thead">
        <vs-th sort-key="catatan">Nama</vs-th>
        <vs-th sort-key="tipe">Kategori</vs-th>
        <vs-th sort-key="tipe">Jumlah</vs-th>
      </template>
      ...
      <template slot-scope="{data}">
        <tbody>
          <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data">
            ...
            <vs-td>
              <p class="product-jumlah">{{ tr.category_name }}</p>
            </vs-td>

            <vs-td>
              <p class="product-jumlah">{{ tr.jumlah }}</p>
            </vs-td>
...
import moduledataLaporanProduk from '@/store/laporan/details/produk/moduleLaporanProduk.js' //penamaan moduleDataSupplier diganti sesual menui
...
export default {
  ...
  computed: {
    ...
    products () { //ganti suppliers dengan menu yang baru
      return this.$store.state.dataProduct.products //ganti suppliers dengan menu yang baru | dataSupplier wajib sama dengan bagian created > register module | ganti nama supplierswajib sama dengan state.js nya
    },
    queriedItems () {
  ...
  created () {
    if (!moduledataLaporanProduk.isRegistered) {
      this.$store.registerModule('dataProduct', moduledataLaporanProduk) //dataSupplier ganti dengan dataNamamenu
      moduledataLaporanProduk.isRegistered = true
    }
    const id = {
      idBusiness : this.$store.state.AppActiveUser.id_business
    }
    this.$store.dispatch('dataProduct/fetchDataProdukItems', id)
  },
  mounted () {
    this.isMounted = true
  }
