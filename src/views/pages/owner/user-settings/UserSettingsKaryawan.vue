...
<template>
  <div id="data-list-thumb-view" class="data-list-container">
    ...
    <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage" search :data="employees">
      ...
      <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
        ...
        <div class="flex flex-wrap-reverse items-center">
          ...
          <!-- ACTION - DROPDOWN -->
          <vs-button @click="deleteMultiple()" class="p-3 mb-4 mr-4 rounded-lg cursor-pointer shadow-md flex items-center justify-between text-lg font-medium text-base text-white bg-danger">Hapus Karyawan</vs-button>
      ...
      <template>
        ...
        <div class="vx-row mb-6">
          <div class="vx-col sm:w-1/3 w-full">
            <span>Nama Karyawan<sup class="text-danger">*</sup></span>
          </div>
        ...
        <div class="vx-row my-3">
          ...
          <ul class="centerx flex">
             <li class="mr-4">
              <vs-radio v-model="role" vs-value="cashier">Cashier</vs-radio>
            </li>
            <li>
              <vs-radio v-model="role" vs-value="courier">courier</vs-radio>
            </li>
          </ul>
          </div>
        </div>
        <div class="vx-row mb-6">
          <div class="vx-col sm:w-1/3 w-full">
            <span>No. Hp Karyawan<sup class="text-danger">*</sup></span>
          </div>
          <div class="vx-col sm:w-2/3 w-full">
        ...
        <div class="vx-row mb-6">
          ...
          <div class="vx-col sm:w-2/3 w-full">
            <vs-input type="email" class="w-full" icon-pack="feather" icon="icon-mail" name="item-email" icon-no-border v-model="email" v-validate="'required|email'"/>
        </div>
        </div>
        <div class="vx-row mb-6">
          <div class="vx-col sm:w-1/3 w-full mt-6">
        ...
        <div class="vx-row mb-6">
          ...
          <div class="vx-col sm:w-2/3 w-full">
            ...
            <span class="text-danger text-sm" v-show="errors.has('item-alamat')">{{ errors.first('item-alamat') }}</span>
        </div>
          </div>
        <div class="vx-row">
...
export default {
  ...
  data () {
    return {
      ...
      validations: {
        email: { required: true, 
          checkEmail: (id, vm, email) => {
            return vm.$http.post(`/api/v1/settings/employee/${id.idBusiness}/show`, {email: [email]})
          }
        }
      }
    }
  ...
  computed: {
    ...
    employees () { //ganti suppliers dengan menu yang baru
      return this.$store.state.dataEmployee.employees //ganti suppliers dengan menu yang baru | dataSupplier wajib sama dengan bagian created > register module | ganti nama supplierswajib sama dengan state.js nya
    },
    queriedItems () {
      return this.$refs.table ? this.$refs.table.queriedResults.length : this.employees.length //ganti suppliers dengan menu yang baru
    }
  },
  methods: {
    deleteMultiple () {
    ...
    acceptMultipleDelete () {
      for (let index = 0; index < this.selected.length; index++) {
        this.$store.dispatch('dataEmployee/removeItem', this.selected[index].id).catch(err => { console.error(err) })
      }
      this.$vs.notify({
        color: 'success',
        ...
        if (result) {
          ...
          if (this.id !== null && this.id >= 0) {
            ...
            this.$vs.notify({
              ...
              text: `Data ${this.title} ${obj.karyawans} berhasil ditambahkan` // obj.supplier ini bisa di ganti dengan variable di array obj di atas, wajarnya yang menginisialkan nama. contoh obj.nama
            })
          } else {
            delete obj.id
    ...
    deleteData (data) {
      ...
      this.$vs.dialog({
        ...
        text: `Apakah Anda serius ingin menghapus data ${this.title} atas nama "${data.karyawans}"`, //data.supplier, supplier ini di ganti dengan salah satu variable di row table yang menunjukan nama
