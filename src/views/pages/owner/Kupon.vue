    ...
    <vs-table ref="table" multiple v-model="selected" pagination :max-items="itemsPerPage" search :data="coupons">
      ...
      <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
        ...
        <div class="flex flex-wrap-reverse items-center">
          ...
          <!-- ACTION - DROPDOWN -->
          <vs-button @click="deleteMultiple()" v-if="selected.length > 0 || selected.length < 0" class="p-3 mb-4 mr-4 rounded-lg cursor-pointer shadow-md flex items-center justify-between text-lg font-medium text-base text-white bg-danger">Hapus Kupon</vs-button>
    ...
    <vs-popup class="crud uppercase" :title="titlePopup" :active.sync="popupActivo">
      <template>
        ...
        <div class="vx-row mb-6">
          ...
          <div class="vx-col sm:w-2/3 w-full">
          <vue-numeric currency="Rp. " separator="." v-model="diskon" class="vs-inputx vs-input--input normal" v-validate="'required'" name="diskon"></vue-numeric>
...
<script>
import moduleDataCoupon from '@/store/kupon/moduleKupon.js' //penamaan moduleDataSupplier diganti sesual menui
  ...
  methods: {
    ...
    acceptMultipleDelete () {
      for (let index = 0; index < this.selected.length; index++) {
        this.$store.dispatch('dataKupon/removeItem', this.selected[index].id).catch(err => { console.error(err) })
      }
      this.$vs.notify({
        color: 'success',
